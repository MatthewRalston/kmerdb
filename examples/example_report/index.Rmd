...
```{r fig.cap="Similarity between individual, unnormalized distributions and the total distribution", fig.width=8.41, fig.height=6.5, out.extra='angle=90'}
...
```{r fig.cap="Skewness-kurtosis analysis for distribution fitting of the count model across all 11 species."}
library(fitdistrplus)
...
```{r fig.cap="Alternate distribution showing Poisson model (green) and Negative binomial fit (red)", fig.width=8.41, fig.height=6.5, out.extra='angle=90'}
d5 <- NULL
for (s in columns){
...
ggplot(data2) + geom_density(aes(x=Count)) + geom_line(data=d5, aes(y=Fit, x=x, colour=Distr)) + ggtitle("Comparing model fit for 8-mer count across the Clostridia") + facet_wrap(~Species)
```
...
```{r fig.cap="Is the count dimension of k-mer space log-normal"}
...
```{r fig.cap="Un-normalized counts per sample"}
unnormalized <- data2
...
```{r fig.cap="Normalized counts per sample"}
...
```
Now that we have corrected this size effect through normalization, let's consider whether there remains a size effect in certain species. There is certainly a lot of signal, and even some highly comparable distributions between the Clostridia. However, it's worth reminding ourselves here that the rarest k-mers have the highest probability density throughout the negative binomial. There may be lots of very rare sequences with low abundances and this sequence space has high propensity for change since the abundances don't necessarily reflect the connectivity. That means there can be inflection points amongst neighbors in terms of abundance, and that could be better captured with transcriptomic sample dynamics. So far we have just been looking at the features of the DNA, and not the expressed gene at this point. It happens to be that more of the expressed RNA is downstream of certain promoters and TFBS and we need to understand which motifs are highly abundant or easliy clustered. And of course we could look at those in the RNA-Seq too, but again we're just in the DNA features at this point of the puzzle.
