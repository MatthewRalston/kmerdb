...
```{r fig.cap="Un-normalized counts per sample", echo=FALSE, message=FALSE, eval=TRUE}![The K-means clustering for a spearman distance matrix](kmeans_k3_clustering_on_spearman_dist.png)
...
function getmetagenome(){
  ...
  metagenome=(sed -n "${col}p" | sed 's/.*/&.fna/')
  echo "${metagenome[@]}"
}
export -f getmetagenome
parallel -j 30 'kmerdb profile -k {2} -pg postgres://matt@localhost:5432/kmerdb $(getmetagenome {1})  metagenome_{1}.{2}.kdb 2> metagenome_{1}.{2}.log;' ::: $(seq 1 30) ::: $(seq 8 14)
```
