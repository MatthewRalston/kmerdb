...
```{r fig.cap="Alternate distribution showing Poisson model (green) and Negative binomial fit (red)", fig.width=8.41, fig.height=6.5, out.extra='angle=90', echo=FALSE, eval=FALSE}
d5 <- NULL
for (s in columns){
...
```{r fig.cap="Un-normalized counts per sample", echo=FALSE, message=FALSE, eval=FALSE}
unnormalized <- data2
...
```{r fig.cap="Normalized counts per sample", echo=FALSE, message=FALSE, eval=FALSE}
...
parallel -j 30 "metagenome=($(getmetagenome {1})); kmerdb profile -k {2} ${metagenome[@]} metagenome_{1}.{2}.kdb 2> metagenome_{1}.{2}.log;" ::: $(seq 1 30) ::: $(seq 8 14)
```
