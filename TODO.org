...
** Use the 5 best scoring  (wrt original genome/kdb) minimizers by match count to seed alignment

** More specifically, a match is seeded by the 5 most optimal scores by optimal number of matches of minimizer counts

** match is intersection of minimizer optima with group of max score SW alignment.

** match is restricted to 5 (ln of sequence) or more minimizer matches or next closest (matchmin 4+)
