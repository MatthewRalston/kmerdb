...
** its a regression problem. the coefficients should sum to unity, and the vectors times coeff1 * coeff2? should sum to the composite vector, and unity.
...
** Lots of priorities, currently. Comp. analysis, custom SVD implementation? A = UsigmaVt  

** Markov chain submodules? Log-likelihood ratio

** TODO T O D O  list

*** Quarto journals elsevier preprint for biorxiv

*** compositional analysis (p0)

*** WAITING Are all species downloaded
:LOGBOOK:
