...
* Proecedural plan
** Convert kdb metadata to .tsv (kmer nodes, total_kmers)
** Convert kdb metadata to .tsv (kmer-kmer relationships)
** Convert kdb metadata to .tsv (paths)
* Structural types
** Nodes (:nodeLabel)
*** Objects or entities
**** kmers
...
****** kmer_id
****** kmer_seq
****** seq_id (fastq read-id or Fasta sequence id)
****** start
****** reverse
*** Can be labeled
**** Seed (Path property)
**** IUPAC (k-mer node property)
**** Forward/reverse (a kmer-kmer relationship)
*** May have properties
**** kmer_id
**** kmer_seq
**** seq_id (fastq read-id or Fasta sequence id)
...
** seq_id
** reverse
* Match (relationship: kmer-kmer)
** seq1, seq2
** types:
*** external (a self referential k-mer relationship)
**** seq1 != seq2
**** seq1[start:start+k] == seq2[start:start+k]
*** internal (relating to a neighboring k-mer)
**** seq1 == seq2
**** seq1[start:start+k] <=> seq1[start+1:start+k+1]
