...
def get_expected_codon_frequencies(L:int, codon_counts:pd.DataFrame):
    ...
    :returns: expected codon (frequencies, counts) from sequence of length L
    :rtype: tuple
    """
    if type(L) is not int:
        raise TypeError("kmerdb.codons.get_expected_codon_frequency expects L to be an int")
    elif not isinstance(codon_counts, pd.DataFrame):
        raise TypeError("kmerdb.codons.get_expected_codon_frequency expects codon_count_vector to be an Pandas DataFrame")
    ...
    total_cdn_cnts = np.array(codon_counts.sum(axis=0)) # Column sum of codon_counts across all genes
    ...
    for aa, indices in synonymous_codons.items():
        ...
        #print("\n\nAmino acid: {0}\ntotal counts: {1}\ncounts: {2}".format(aa, family_aa_total_cnt, cdn_cnts_for_aa))
        
        
        family_frequencies = cdn_cnts_for_aa / family_aa_total_cnt
        #print("Amino acid '{0}': \ncounts: {1}\nfamily frequencies {2}".format(aa, cdn_cnts_for_aa, family_frequencies))
