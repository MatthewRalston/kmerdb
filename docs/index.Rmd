...
abstract: |
  ** 
  **Background:** The command-line tool kmerdb is a k-mer counter/sorter. The tool is a specialty format that I'd eventually like to write in Rust. However, the full NumPy suite enables unsupervised learning, biopython, scikit integration, rpy2 hypothesis testing (jk) and multiple test correction, and much more. The goal is to find a stable k-mer storage format that contains the ideal features. The current specification is 5 columns (a single column of k-mer ids and count, frequency). The metadata format is fully specified in kmerdb/config.py.
  The sort order may be interesting from a Spearman perspective. I hypothesize that sort-order is one of the two most important factors for understanding the k-mer problem. Perhaps other people have looked at this already, but it's not guaranteed. I'll take a guess and say that there isn't a tool yet that can look into both Spearman and Pearson correlation matrices of k-mer arrays (either as frequency or as count).
  I'd intend for this to be used for access to k-mer counts only for some time. Interactive upload for select users, contingent upon completion of that feature, is at the literal last of my to-do list until I have support in some way or another. I intend to complete this feature eventually, and it will be in my off-time. 
  **Methodology:** The CLI was written in Python using Bio.bgzf and other dependencies as specified in the packages requirements file. The unsupervised learning features were courtesy of sci-kit learn. The format was designed by me to do two things: store lexical order, and store both count and frequency. 
  **Results:** The command-line provides reasonable access to k-mer profiles stored as flat-files.
...
## kmwesb ia PyPI package for k-mer profiles.
...
From this point, profiles of the same k can be combined with the `kmerdb matrix` command, producing 'unnormalized(pass)' or 'normalized' data matrices. The `kmerdb matrix` command simply collates count vectors, that could have been sorted prior during profile generation. The rank order has not been incorporated in any meaningful way other than direct Spearman distance matrices. Otherwise, it is recommended that Pearson $R^2$ produced from the custom implementation be used to evaluate profile similarity via dimensionality reduction, k-means clustering, PCA, UMAP, t-SNE, or whatever other dimensionality reduction technique is necessary to display the necessary patterns in the data, and hierarchical clustering is the end goal for evaluation of accuracy of this method.
...
We used the simulated metagenomes of firmicutes and proteobacteria of Anvar et al. to test some hypotheses related to microbial diversity in the abstract k-space (\parencite{anvar2014determining}). First we downloaded 


Calculations were performed on a AMD ThreadRipper 3960 with 64Gb RAM, 

```bash
function getmetagenome(){
  col=$1
  metagenome=($(cut -f$col microbes.txt | sed 's/.*/&.fna/'))
